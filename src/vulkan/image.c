...
void transition_layout(vulkan_image* image, VkImageLayout from, VkImageLayout to, VkImageAspectFlags aspects) {
    VkCommandBuffer cmd = vulkan_context_start_recording(image->ctx);
    ...
    if (from == VK_IMAGE_LAYOUT_UNDEFINED && to == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        ...
        FATAL("Unsupported layout transition from %d to %d", from, to);
    }
    ...
    vulkan_context_submit(image->ctx, cmd);
}

void copy_buffer_to_image(vulkan_image* dst, vulkan_buffer* src) {
    VkCommandBuffer cmd = vulkan_context_start_recording(dst->ctx);
